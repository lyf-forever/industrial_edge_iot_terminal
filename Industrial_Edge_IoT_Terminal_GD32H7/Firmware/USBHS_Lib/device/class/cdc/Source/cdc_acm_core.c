...
uint8_t user_usb_rx_buffer[USER_USB_RXBUFF_SIZE];
uint8_t user_usb_usart_rx_buffer[USER_USB_RXBUFF_SIZE];
uint16_t user_usb_usart_rx_state = 0;   
...
//         if ((user_usb_usart_rx_state & 0x8000) == 0)     /* 接收未完成 */
//         {
//             if (user_usb_usart_rx_state & 0x4000)        /* 接收到了0x0D */
//             {
//                 if (res != 0x0a)
//                 {
//                     user_usb_usart_rx_state = 0;         /* 接收错误,重新开始 */
//                 }
//                 else
//                 {
//                     user_usb_usart_rx_state |= 0x8000;   /* 接收完成了 */
//                 }
//             }
//             else                                    /* 还没收到0X0D */
//             {
//                 if (res == 0x0d)
//                 {
//                     user_usb_usart_rx_state |= 0x4000;   /* 标记接收到了0X0D */
//                 }
//                 else
//                 {
//                     user_usb_usart_rx_buffer[user_usb_usart_rx_state & 0X3FFF] = res;
//                     user_usb_usart_rx_state++;

//                     if (user_usb_usart_rx_state > (USER_USB_RXBUFF_SIZE - 1))
//                     {
//                         user_usb_usart_rx_state = 0;     /* 接收数据溢出 重新开始接收 */
//                     }
//                 }
//             }
//         }
...
//     va_end(args);
//     if (len > 0) {
//         usb_cdc_send((uint8_t*)buf, len);
//     }
// }

// // 底层单包发送函数（阻塞等待上一包完成）
// uint16_t user_usb_send(uint8_t *buf, uint16_t length) {
//     if (USBD_CONFIGURED != cdc_acm.dev.cur_status) return 0;
//     if (length == 0) return 0;

//     usb_cdc_handler *cdc = (usb_cdc_handler*)cdc_acm.dev.class_data[CDC_COM_INTERFACE];
//     // 等待上一次发送完成（packet_sent 应在中断中置1）
//     while (cdc->packet_sent == 0);
//     cdc->packet_sent = 0U;

//     // 调用官方库发送函数
//     usbd_ep_send(&cdc_acm, CDC_DATA_IN_EP, buf, length);
//     return length;
// }

// // 上层多包发送函数（自动分包，调用 user_usb_send）
...
//         }
//         // 调用底层单包发送（注意强制转换去掉 const）
//         uint16_t ret = user_usb_send((uint8_t*)(data + sent), chunk);
//         if (ret != chunk) {
//             // 发送失败，可根据需要添加错误处理（如重试或退出）
//             break;
//         }
//         sent += chunk;
//     }
...
// // 在主循环中定期调用，检查并接收来自PC的数据
// void usb_receive_handler(void) {
//     // 1. 检查CDC设备是否就绪
...
//         if (data_len > 0) {
//             // 4. 拷贝数据到应用缓冲区
//             memcpy(user_usb_rx_buffer, cdc->data, data_len);
//             // 5. 可选：清零接收长度标志
