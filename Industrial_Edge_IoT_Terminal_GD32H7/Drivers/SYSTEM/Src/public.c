...
{  
    uint64_t loops=(uint64_t)count*10U;		/* 64 位累加，避免 count*10 溢出；count=0 时不延时 */
    while(loops--)
    {
        delay_us(100);
    }
...
/*!
* 说明     D-Cache 行失效（DMA 写内存后、CPU 读取前调用）
* 参数[1]  addr: 起始地址（无需 32 字节对齐）
* 参数[2]  len : 字节长度
* 返回值   无
*/
